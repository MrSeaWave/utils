...
class Gradient {
  ...
  constructor(colors, count = 100) {
    if (!Array.isArray(colors)) {
      throw new Error(`constructor param must be Array, current param is ${colors}`);
    }
    if (colors.length < 2) {
      throw new Error(`length of constructor param must >= 2, current length is ${colors.length}`);
    }
    this._colors = colors;
    this._width = count;
    this[INIT]();
  ...
  [DRAW_GRADIENT]() {
    ...
    const gradient = _ctx.createLinearGradient(0, this._height / 2, this._width, this._height / 2);
  ...
  [GET_COLORS]() {
    const { _ctx } = this;
    const imageData = _ctx.getImageData(0, this._height / 2, this._width, 1).data;
